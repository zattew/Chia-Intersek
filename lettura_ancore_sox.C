...
void lettura_ancore_sox (ifstream &prima, ifstream &seconda ,vector<string> &gene, vector<int> &enhancer_associati, int dim_mappa, int uno, vector<string> &S ) 
{
	...
	while ( getline ( prima, linea ) )
	{
		...
		for ( int i = 0; i < vettore_punteggio.size(); i++)
		{
			if ( vettore_punteggio[i] == 0 )
			{
				int valore_iniziale = (temp[i] - 1);
				
				//Solo se l'ancora ha collegato una S allora questa è un enhancer con Sox2
				for ( int x = 0; x < S.size(); x++)
					if ( x == valore_iniziale && S[x] == "S" )
						enh++;
			}
		}
